  ...
  return (
    <div className="flex gap-0.5">
      {Array.from({ length: max }).map((_, i) => (
        <svg
          key={i}
          className={`${cls} ${i < Math.floor(rating) ? "text-amber-400" : i < rating ? "text-amber-300" : "text-white/15"}`}
      ...
      >
        ...
        <div className="flex gap-3 flex-1 min-h-0">
          ...
          <div className="flex flex-col items-center justify-center shrink-0 w-14">
            <span className="text-[28px] font-bold text-white leading-none">{ratingLabel}</span>
            <span className="text-[9px] text-white/40 mt-0.5">/ 5</span>
            <div className="mt-1.5">
              <StarRating rating={avg_rating} size="sm" />
            </div>
            {total > 0 && (
              <span className="text-[8px] text-white/40 mt-1.5 text-center leading-tight">
          ...
          <div className="flex-1 min-w-0 flex flex-col justify-center space-y-1.5">
            {distribution.map(d => (
              <div key={d.stars} className="flex items-center gap-1.5">
                <span className="text-[10px] text-white/50 shrink-0 w-3 text-right">{d.stars}</span>
                <div className="flex-1 h-1.5 bg-white/8 rounded-full overflow-hidden">
                  <div className="h-full bg-amber-400 rounded-full" style={{ width: `${d.pct}%` }} />
                </div>
                <span className="text-[10px] text-white/50 w-8 text-right shrink-0 tabular-nums">{d.pct}%</span>
              </div>
            ))}
          </div>
    ...
    <div onClick={onClick} className={`bg-[#1a2744] border border-white/10 rounded-xl p-4 ${clickable}`}>
      ...
      <div className="flex gap-6 mb-4">
        <div className="shrink-0 text-center min-w-[72px]">
          <div className="text-5xl font-bold text-white leading-none">{ratingLabel}</div>
          <div className="text-xs text-white/40 mb-1.5">/ 5</div>
        ...
        <div className="flex-1">
          <div className="text-[11px] text-white/50 font-medium mb-2">Rating Distribution</div>
          <div className="space-y-1.5">
            {distribution.map(d => (
              <div key={d.stars} className="flex items-center gap-2">
                <span className="text-[11px] text-white/60 w-3 shrink-0 text-right">{d.stars}</span>
                <div className="flex-1 h-2 bg-white/8 rounded-full overflow-hidden">
                  <div className="h-full bg-amber-400 rounded-full" style={{ width: `${d.pct}%` }} />
                </div>
                <span className="text-[11px] text-white/60 w-10 text-right shrink-0">{d.pct}%</span>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Two-column themes */}
      <div className="border-t border-white/8 pt-3 grid grid-cols-2 gap-x-6">
        <div>
          <div className="text-[10px] font-bold text-green-400 uppercase tracking-wide mb-2">
            Top Positive Themes
          </div>
          <div className="space-y-1.5">
            {top_positive_topics.length > 0
              ? top_positive_topics.slice(0, 5).map(t => (
                  <div
                    key={t.label}
                    className={`flex items-center justify-between gap-2 rounded px-1 -mx-1 ${onThemeClick ? "cursor-pointer hover:bg-green-500/10" : ""}`}
                    onClick={onThemeClick ? (e) => { e.stopPropagation(); onThemeClick(t.label); } : undefined}
                  >
                    <span className="text-[12px] text-white/60 truncate">{t.label}</span>
                    <span className="text-[12px] font-semibold text-green-400 shrink-0">{t.pct}%</span>
                  </div>
                ))
              : <span className="text-[10px] text-white/40">No data yet</span>
            }
          </div>
        </div>
        <div>
          ...
          <div className="space-y-1.5">
            {top_negative_topics.length > 0
              ? top_negative_topics.slice(0, 5).map(t => (
                  <div
                    key={t.label}
                    className={`flex items-center justify-between gap-2 rounded px-1 -mx-1 ${onThemeClick ? "cursor-pointer hover:bg-red-500/10" : ""}`}
                    onClick={onThemeClick ? (e) => { e.stopPropagation(); onThemeClick(t.label); } : undefined}
                  >
                    <span className="text-[12px] text-white/60 truncate">{t.label}</span>
                    <span className="text-[12px] font-semibold text-red-400 shrink-0">{t.pct}%</span>
                  </div>
                ))
              : <span className="text-[10px] text-white/40">No data yet</span>
            }
