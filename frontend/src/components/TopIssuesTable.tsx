...
/* ── Split bar: pos | neg | neutral within the volume bar ── */
function SplitBar({ width, posPct, negPct }: { width: number; posPct: number; negPct: number }) {
  const neuPct = Math.max(0, 100 - posPct - negPct);
  return (
    <div className="h-2 rounded-full bg-white/5 overflow-hidden">
      <div className="flex h-full rounded-full overflow-hidden" style={{ width: `${width}%` }}>
        {posPct > 0  && <div className="bg-emerald-500/70" style={{ width: `${posPct}%` }} />}
        {neuPct > 0  && <div className="bg-white/20"       style={{ width: `${neuPct}%` }} />}
        {negPct > 0  && <div className="bg-red-500/70"     style={{ width: `${negPct}%` }} />}
      </div>
    </div>
  );
}
...
function ClusterRow({ c, maxCount }: { c: IssueCluster; maxCount: number }) {
  ...
  const netColor = isNeg ? "text-red-400" : "text-emerald-400";
    ...
    <div>
      <div className="flex items-center justify-between gap-2 mb-1">
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          {c.trend === "rising" && (
            <span className="text-[9px] bg-amber-500/15 text-amber-400 border border-amber-500/20 rounded px-1 py-px font-semibold shrink-0">
              ↑ Rising
            </span>
          )}
        </div>
      ...
      <SplitBar width={barPct} posPct={posPct} negPct={negPct} />
    </div>
  );
}
  ...
  return (
    <div className={`pl-2 border-l-2 ${accent}`}>
      <div className="flex items-center justify-between gap-2 mb-1">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="text-[10px] text-emerald-400">{posPct}%▲</span>
          <span className="text-[10px] text-red-400">{negPct}%▼</span>
...
export function TopIssuesTable({ brandId, compact, onClick, onClusterClick, onCategoryClick }: Props) {
  ...
  const [sentFilter, setSentFilter] = useState<SentimentFilter>("negative");
  ...
  /* ── Compact ── */
  if (compact) {
    const filtered = hasClusters
      ? clusters.filter(c =>
          sentFilter === "negative" ? c.net_sentiment_pct < 0 :
          sentFilter === "positive" ? c.net_sentiment_pct >= 0 : true
        ).slice(0, 6)
      : [];
    const maxCount = Math.max(filtered[0]?.article_count ?? clusters[0]?.article_count ?? 1, 1);
      ...
      >
        ...
        <div className="flex items-center justify-between mb-1.5 flex-none">
          ...
          <div className="flex items-center gap-0.5 bg-white/5 rounded-md p-0.5">
            {(["negative", "all", "positive"] as SentimentFilter[]).map(f => (
              <button
                key={f}
                onClick={e => { e.stopPropagation(); setSentFilter(f); }}
                className={`text-[9px] px-1.5 py-0.5 rounded transition-colors font-medium capitalize ${
                  sentFilter === f
                    ? f === "negative" ? "bg-red-500/25 text-red-300"
                    : f === "positive" ? "bg-emerald-500/25 text-emerald-300"
                    : "bg-white/15 text-white/80"
                    : "text-white/35 hover:text-white/60"
                }`}
              >
                {f === "negative" ? "↓ Neg" : f === "positive" ? "↑ Pos" : "All"}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between mb-1 pb-1 border-b border-white/8 flex-none">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <span className="text-[9px] font-semibold text-white/40 uppercase tracking-wide">Vol</span>
            <span className="text-[9px] font-semibold text-white/40 uppercase tracking-wide w-10 text-right">Net</span>
          </div>
        </div>
        ...
        {clustersLoading ? (
          ...
          <div className="flex-1 min-h-0 overflow-hidden space-y-2">
            {filtered.map(c => {
              ...
              return (
                <div
                  key={c.cluster_name}
                  className={onClusterClick ? "cursor-pointer" : ""}
                ...
                >
                  <div className="flex items-center justify-between gap-2 mb-0.5">
                    <div className="flex items-center gap-1 min-w-0">
                      ...
                      {c.trend === "rising" && (
                        <span className="text-[9px] text-amber-400 font-semibold shrink-0">↑</span>
                      )}
                    </div>
          ...
          <div className="flex-1 flex items-center justify-center">
            <span className="text-[9px] text-white/40 text-center">
              No {sentFilter} issues in this period
            </span>
          </div>
        ) : (
          <div className="flex-1 flex items-center justify-center">
            <span className="text-[9px] text-white/40 text-center">Populates after next pipeline run</span>
          </div>
        )}
      </div>
    );
